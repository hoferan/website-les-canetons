 ...
 *
 * THE LABEL IS VISUALLY HIDDEN AND REPEATED AS THE PLACEHOLDER. A visible label
 * above the box would cost the planning a row, which is what #182 spent two
 * rounds taking away; the magnifier and the placeholder say what the box is
 * to a sighted reader, and the <label> says it to everybody else. A
  ...
  return (
    <div className={`relative ${className ?? ""}`}>
      ...
      <Input
        ...
        // Room for the magnifier on the left and the clear button on the
        // right. WebKit's own cancel button is hidden: two clear buttons, one
        // of which only some browsers draw, is one too many. `bg-panel` and
        // `border-line` because it sits beside the roster's native selects,
        // and Input's transparent default showed the page grey through it.
        className="border-line bg-panel pr-12 pl-9 [&::-webkit-search-cancel-button]:hidden"
        autoComplete="off"
      />
      {value === "" ? null : (
        <Button
