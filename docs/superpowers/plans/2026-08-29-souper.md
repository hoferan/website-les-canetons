...
6. **`tsconfig.json` sets `noUncheckedIndexedAccess: true`**, so `getAllByRole(...)[0]` is `HTMLElement | undefined` and will not typecheck where an `Element` is wanted. Every array index in this plan's test snippets therefore carries a `!`; that is the house idiom (see `web/src/pages/PlanningRepet.test.tsx:35`), not sloppiness. **Run `npm run typecheck`, not only vitest** — a compile error shipped behind three green test runs earlier in this plan's own execution, because tests passing and code compiling are different claims.
