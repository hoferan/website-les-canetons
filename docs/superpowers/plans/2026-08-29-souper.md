...
type Counts = { meat: number; child: number; vegetarian: number };
...
export function SignupsAdmin() {
  ...
  // customFetch throws on every non-2xx before a query resolves, so by here the
  // status is always 200; this proves it to the type checker rather than
  // asserting past it. Same shape as InscriptionsAdmin.
  const data = summary.data.data as {
    totalPersons: number;
    totalTables: number;
    menuTotals: Counts;
    tables: {
