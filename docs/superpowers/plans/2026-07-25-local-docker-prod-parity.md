...
1. ~~**The deny-all hardening is not reachable by a direct request.**~~ **This "correction" was itself wrong — the spec's original claim was right.** It asserted that `app/.htaccess`'s catch-all rewrites every non-`/assets/` URL to `index.php` *before* authorization runs, so `GET /api-laravel/.env` would resolve to the old app and answer 404. The phase order is the other way round: authorization is evaluated during Apache's **directory walk**, before mod_rewrite's per-directory rules run in the **fixup** phase. So the catch-all never sees a request that resolves under a denied directory.

   Verified against the running stack in Task 7: `/api-laravel/.env`, `/api-laravel/vendor/autoload.php` and even a nonexistent path under the tree all return **403** (`authz_core` `AH01630`), not 404. `Require all denied` is the load-bearing protection, not defense in depth.

   This makes prerequisite 1 (the deploy CLI never uploading these files) more serious than first framed: the Laravel tree on TEST/QA/PROD has no protection at all, and no catch-all backstop for paths that resolve to real files. The smoke assertions stay as written ("not 200, and no secret material in the body") — they are correct for either status and survive the 2a-ii catch-all change.
